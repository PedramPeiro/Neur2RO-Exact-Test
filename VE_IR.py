...
def _add_constraints(
    mdl: gp.Model, X, Y, t, N, M, K, z_vertices, opening_cost, capacity, D_bar, D_hat, ship_cost):
    """
    Add constraints for vertex-enumeration ARO with IR:
...
def main() -> None:
    ...
    if args.Gamma is None:
        raise ValueError(
            "Gamma (budget of uncertainty) must be provided for VE_IR. "
    ...
    mdl = utils.init_model(
        name=f"MIP_VE_IR_{args.instance}_G{args.Gamma}",
    ...
    meta = {
        "header": (
            f"Result - instance {args.instance} (IR, VE, Gamma={args.Gamma})\n"
        ...
        "n_customers": len(M),
        "method": "VE",
        "uncertainty": f"IR_VE_Gamma_{args.Gamma}",
    ...
    # utils.write_summary(summary, run_dir / "summary_VE_IR.csv")

    print(f"[VE_IR] run complete → {txt_log}")
