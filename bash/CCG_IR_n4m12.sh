...
#SBATCH --time=2:00:00
#SBATCH --job-name=CCG_IR_n4m12
#SBATCH --output=CCG_IR_n4m12_%j.out
...
cat > run_CCG_IR_n4m12_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_CCG_IR_n4m12_worker.sh
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/CCG_IR_n4m12.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_CCG_IR_n4m12_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All CCG_IR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/CCG_IR_n4m12.log || true
