...
#SBATCH --time=2:00:00
#SBATCH --job-name=CCG_IR_n5m15
#SBATCH --output=CCG_IR_n5m15_%j.out
...
cat > run_CCG_IR_n5m15_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
"$HOME/.venvs/exact2ro/bin/python" -u CCG_IR.py \
  --instance "${instance}" \
  --time_limit 1800 \
...
chmod +x run_CCG_IR_n5m15_worker.sh

# ===== Instance & Gamma lists =====
INSTANCES=(IR_n5_m15_rep{0..29})
GAMMAS=(1 2 3 4 15)
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/CCG_IR_n5m15.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_CCG_IR_n5m15_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All CCG_IR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/CCG_IR_n5m15.log || true
