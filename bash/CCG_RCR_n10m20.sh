...
#SBATCH --time=2:00:00
#SBATCH --job-name=CCG_RCR_n10m20
#SBATCH --output=CCG_RCR_n10m20_%j.out
...
# ===== Inline worker script (one instance, one Gamma) =====
cat > run_CCG_RCR_n10m20_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
"$HOME/.venvs/exact2ro/bin/python" -u CCG_RCR.py \
  --instance "${instance}" \
  --time_limit 1800 \
...
chmod +x run_CCG_RCR_n10m20_worker.sh

# ===== Instance & Gamma lists =====
INSTANCES=(RCR_n10_m20_rep{0..29})
GAMMAS=(1 2 3)
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/CCG_RCR_n10m20.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_CCG_RCR_n10m20_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All CCG_RCR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/CCG_RCR_n10m20.log || true
