...
#SBATCH --time=2:00:00
#SBATCH --job-name=CCG_RCR_n5m10
#SBATCH --output=CCG_RCR_n5m10_%j.out
...
cat > run_CCG_RCR_n5m10_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_CCG_RCR_n5m10_worker.sh

# ===== Instance & Gamma lists =====
INSTANCES=(RCR_n5_m10_rep{0..29})
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/CCG_RCR_n5m10.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_CCG_RCR_n5m10_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All CCG_RCR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/CCG_RCR_n5m10.log || true
