...
#SBATCH --time=2:00:00
#SBATCH --job-name=VE_RCR_n4m12
#SBATCH --output=VE_RCR_n4m12_%j.out
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/VE_RCR_n4m12.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_VE_RCR_n4m12.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All VE_RCR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/VE_RCR_n4m12.log || true