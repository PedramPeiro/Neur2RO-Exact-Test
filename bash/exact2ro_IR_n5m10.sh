...
cat > run_exact2ro_IR_n5m10_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_exact2ro_IR_n5m10_worker.sh
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/exact2ro_IR_n5m10.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --ungroup \
  ./run_exact2ro_IR_n5m10_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"
parallel_exit=$?
set -e

if (( parallel_exit != 0 )); then
    echo "WARNING: GNU parallel reported failures (exit code = ${parallel_exit})."
    echo "Check TrilliumResults/exact2ro_IR_n5m10.log for which jobs failed."
else
    echo "All exact2ro_IR instances processed successfully (up to ${NPROCS} concurrent)."
fi

tail -n 20 TrilliumResults/exact2ro_IR_n5m10.log || true
