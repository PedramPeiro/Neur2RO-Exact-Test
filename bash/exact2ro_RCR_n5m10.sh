...
#SBATCH --job-name=Exact2RO_RCR_n5m10
#SBATCH --output=Exact2RO_RCR_n5m10_%j.out
...
cat > run_exact2ro_RCR_n5m10_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_exact2ro_RCR_n5m10_worker.sh
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/exact2ro_RCR_n5m10.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_exact2ro_RCR_n5m10_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All exact2ro_RCR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/exact2ro_RCR_n5m10.log || true
