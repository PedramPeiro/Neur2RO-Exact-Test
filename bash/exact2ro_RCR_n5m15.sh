...
#SBATCH --time=3:00:00
#SBATCH --job-name=Exact2RO_RCR_n5m15
#SBATCH --output=Exact2RO_RCR_n5m15_%j.out
...
cat > run_exact2ro_RCR_n5m15_worker.sh << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_exact2ro_RCR_n5m15_worker.sh
...
parallel -j "${NPROCS}" --verbose \
  --joblog TrilliumResults/exact2ro_RCR_n5m15.log \
  --results TrilliumResults/parallel_out/{#}_{1}_G{2} \
  --wd "$PWD" \
  --halt now,fail=1 \
  --ungroup \
  ./run_exact2ro_RCR_n5m15_worker.sh {1} {2} ::: "${INSTANCES[@]}" ::: "${GAMMAS[@]}"

echo "All exact2ro_RCR instances processed (up to ${NPROCS} concurrent)."
tail -n 20 TrilliumResults/exact2ro_RCR_n5m15.log || true
